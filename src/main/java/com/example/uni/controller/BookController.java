...
public class BookController {
    ...
    // 책 판매 글 등록
    @PostMapping
    public ResponseEntity<Void> save(@RequestBody BookDto bookDto, @RequestParam("userId") String userId) {
    ...
    // 글 변경
    @PatchMapping("/{id}")
    public ResponseEntity<Void> update(@RequestBody BookDto bookDto, @PathVariable Long id, @RequestParam("userId") String userId) {
        bookService.update(bookDto, id, userId);
        return ResponseEntity.status(HttpStatus.OK).build();
    }

    @DeleteMapping("/{id}")
    public ResponseEntity<Void> delete(@PathVariable Long id, @RequestBody BookDto bookDto, @RequestParam("userId") String userId) {
